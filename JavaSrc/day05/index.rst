=============================
Day 05
=============================

.. contents:: 
   :depth: 1
   :local:

예제 코드
=========================

#. 영역 합
- `반복 + 메모 <https://github.com/prolecture/problems/blob/master/JavaSrc/day05/영역합.java>`_

#. 오른쪽 아래 이동
- `재귀 + 메모 <https://github.com/prolecture/problems/blob/master/JavaSrc/day05/오른쪽아래이동_재귀.java>`_
- `반복 + 메모 <https://github.com/prolecture/problems/blob/master/JavaSrc/day05/오른쪽아래이동.java>`_

#. 최소 이동 거리
- `재귀 + 메모(교재) <https://github.com/prolecture/problems/blob/master/JavaSrc/day05/최소이동거리.java>`_
- `반복 + 메모 <https://github.com/prolecture/problems/blob/master/JavaSrc/day05/최소이동거리_반복.java>`_

#. 행렬 곱셈
- `재귀 + 반복 <https://github.com/prolecture/problems/blob/master/JavaSrc/day05/행렬곱셈.java>`_


추가
-----------------------

.. toctree::   
   :maxdepth: 1  
   :titlesonly:   
   
   day05sol
   
   

